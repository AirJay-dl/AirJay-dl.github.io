...
import {tournamentUpdates} from '../content/tournament-updates.ts';
import {rankingSnapshot} from '../content/data.ts';
...
const fold=line=>{const parts=[];let segment='',bytes=0;for(const char of line){const n=Buffer.byteLength(char);if(bytes+n>73){parts.push(segment);segment=' ';bytes=1;}segment+=char;bytes+=n;}parts.push(segment);return parts.join('\r\n')};
