...
const verifiedResults=parsed.filter(item=>item.complete).map(({round,playerOne,score,playerTwo})=>({round,playerOne,score,playerTwo}));
const liveMatches=parsed.filter(item=>item.started).map(({round,playerOne,score,playerTwo})=>({round,playerOne,score,playerTwo}));
