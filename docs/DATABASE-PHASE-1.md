...
- Shared JSON path: `/srv/snookercalendar/shared/data/`
- Public website: Nginx serves `/data/` from that shared directory, while pages continue to request `/data/live-score.json` and `/data/rankings.json`
...
2. Initialise the database once with the site-specific Node runtime and `SNOOKER_DB_PATH=/srv/snookercalendar/shared/snooker.db`.
3. Add the database path and explicit shared JSON output paths to the score and ranking scheduled jobs.
...
If the database environment variable is absent, both update jobs behave exactly as before and only publish JSON. A database write error is logged but does not block the current JSON refresh. This makes the migration reversible and prevents a database issue from interrupting score or ranking updates.
