...
export function ResultsHub({initialData}:{initialData:ResultsData}){
  ...
  return <>
    ...
    {live.length>0&&<section className="results-section"><div className="section-bar"><div><p className="eyebrow">IN PLAY</p><h2>Snooker live scores</h2></div><span>Source checked every 15 minutes</span></div><div className="match-list">{live.map(match=><div className="match-row live-match-row" key={`${match.round}-${match.playerOne}`}><span>{match.round}</span><span className="match-player">{match.playerOne}</span><strong className="match-score">{match.score}</strong><span className="match-player">{match.playerTwo}</span></div>)}</div></section>}
    ...
    <aside className="results-method"><strong>Data source: <a href={data.source}>{data.sourceLabel} ↗</a></strong><p>The score source is checked every 15 minutes during active events. This page checks our latest saved snapshot every minute; that does not mean the source updates every minute. “Live” appears only when the source reports a match in progress; otherwise this page presents the latest verified scores and scheduled matches.</p></aside>
  </>;
}
