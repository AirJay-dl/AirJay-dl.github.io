...
const perRound=5;
const categoryQuestions=(id:QuizCategory)=>quizQuestions.filter(q=>q.category===id);
const setCount=(id:QuizCategory)=>Math.ceil(categoryQuestions(id).length/perRound);
...
export function QuizGame(){
  ...
  function nextRound(){
    if(!category)return;
    const next=(round+1)%setCount(category);
  ...
  if(!category)return <section className="quiz-chooser" aria-labelledby="choose-quiz"><div className="section-bar"><div><p className="eyebrow">CHOOSE YOUR CHALLENGE</p><h2 id="choose-quiz">Three ways into the game</h2></div><span>No timer · Play at your pace</span></div><div className="quiz-category-grid">{quizCategories.map(item=><article key={item.id} className="quiz-category-card"><span className="quiz-number">{item.number}</span><p className="eyebrow">{item.kicker}</p><h3>{item.title}</h3><p>{item.description}</p><label htmlFor={`quiz-set-${item.id}`}>Choose a five-question set</label><select id={`quiz-set-${item.id}`} value={blocks[item.id]} onChange={event=>setBlocks(value=>({...value,[item.id]:Number(event.target.value)}))}>{Array.from({length:setCount(item.id)},(_,i)=>i).map(value=><option key={value} value={value}>Set {value+1} · {categoryQuestions(item.id)[value*perRound]?.difficulty||'Beginner'}</option>)}</select><div className="quiz-card-footer"><span>{categoryQuestions(item.id).length} questions</span><button type="button" onClick={()=>start(item.id)}>Start quiz</button></div></article>)}</div><p className="quiz-source-note">Every answer includes a short explanation and a source. Questions are editorially checked; the quiz is free and requires no account.</p></section>;
